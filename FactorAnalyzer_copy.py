...
def get_daily_select_returns(data_df,select_daily_df,tax = 0,select_period = 1):
    ...
    select_daily_df = get_factors_select_period(select_daily_df,select_period,False)
    ...
    for j in range(len(select_idx)):
        date,fac,rank,rtn = select_idx[j]
        next_date = select_idx[j+1][0] if j+1 < len(select_idx) else date + timedelta(days=1)
        
        select_ = (data_df[fac] == rank) & (data_df['date'] >= date) & (data_df['date'] < next_date) 
        ...
        for t in select_time:
            ...
            if t not in select_returns_.keys():
                ...
                continue
    select_returns = pd.Series(select_returns_)
...
def get_select_returns_each(data_df,select_df,tax = 0,select_period = 1):
    '''
    summary:
        根据select_df计算一段时间的某一个因子的long或short的returns
    parameter:
        data_df: 因子和收益数据(收益都乘以了100)
        select_df: 因子选择结果
        select_period: 选择周期
    return:
        select_returns: 选出的每个因子的收益(dataframe)
    '''
    return_df = pd.DataFrame()
    columns = []
    for i in range(len(select_df)):
        df = get_select_returns(data_df,select_df.iloc[i:i+1,:],tax,select_period)
        fac,rank,rtn = select_df.index[i]
        columns.append(f'{fac}_{rank}_{rtn}')
        return_df = pd.concat([return_df,df],axis=1,join='outer')
...
def get_daily_select_returns_each(data_df,select_daily_df,tax = 0,select_period = 1):
    
    select_daily_df = get_factors_select_period(select_daily_df,select_period,False)
    count = select_daily_df.groupby(level=0)['mean_rtn'].count()[0]
    return_df = pd.DataFrame()
    data_df['date'] = data_df.index.date
    tax *= 100

    for i in range(count):
        select_df = select_daily_df.iloc[i::count,:]
        select_idx = select_df.index
        df_ = pd.DataFrame()
        for j in range(len(select_idx)):
            date,fac,rank,rtn = select_idx[j]
            ...
            select_time = data_df[select_].index
            # time_ = re.findall(r'\d+',rtn)[-1]
            select = data_df.loc[select_time,rtn] - tax
            # select.index = data_df.loc[select_time,f'future_{time_}_exit_dt']
            select.index = pd.to_datetime(select.index)
            select.index.name = 'datetime'

            df_ = pd.concat([df_,select],axis=0)
            df_.index.name = 'datetime'
            # df_.reset_index(inplace=True)
            # df_.drop_duplicates(subset ='datetime',keep = 'first',inplace=True)
            # df_.set_index('datetime',inplace=True)
        df_.index = pd.to_datetime(df_.index)
        df_.sort_index(inplace=True)
        return_df = pd.concat([return_df,df_],axis=1,join='outer')
    return_df.columns = [f'factor_{i}' for i in range(count)]

    return return_df / 100
...
def get_factors_select_period(select_df,select_period = 1,if_duplicate = True):
    '''
    summary:
...
def create_pyfolio_input_each(factors,rtns,factor_select = None,daily_factor_select = None,tax = 0,select_period = 1):
    '''
    summary:
        生成pyfolio需要的returns数据，利用pyfolio制作回测报告，可保存为html格式
        factor_select和daily_factor_select为两种因子选择方式，可选其一,也可以都选
        对排名1，2，3等的因子分别计算returns，不合并
    parameter:
    ...
    return:
        ...
        (factor_select和daily_factor_select只有一个不为None): returns_select or returns_daily_select
            因子选择结果的returns (dataframe: factor1_long,factor1_short,factor1_total,factor2_long,factor2_short,factor2_total...)
    ...
    if factor_select is not None and daily_factor_select is not None:
        ...
        returns_select = create_pyfolio_input_each(factors,rtns,factor_select = factor_select,tax = tax,select_period = select_period)
        returns_daily_select = create_pyfolio_input_each(factors,rtns,daily_factor_select = daily_factor_select,tax = tax,select_period=select_period)
    ...
    if factor_select is not None:
        f_s = factor_select
        func_select = get_select_returns_each
    elif daily_factor_select is not None:
        f_s = daily_factor_select
        func_select = get_daily_select_returns_each
    ...
    if 'long' in f_s.keys():
        ...
        returns_long.columns = [f'{i}_long' for i in returns_long.columns]
    ...
    if 'short' in f_s.keys():
        ...
        returns_short.columns = [f'{i}_short' for i in returns_short.columns]
...
def _fig_to_base64(fig):  # 将图片转换为base64编码
    ...
    """
    # Save the figure as a binary PNG image
    imgdata = io.BytesIO()
